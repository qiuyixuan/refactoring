...
public class Sudoku{
    ...
    public void time_delay(int delay){
        if( delay > 0 ) {
            try {
                Thread.sleep(delay);
            }
    ...
    public boolean fail(CellStack stack, int time){
        if (stack.size() == 0){
            System.out.println("time is:" + time);
            System.out.println(stack.size());
    ...
    public CellStack stucked(CellStack stack){
        boolean stuck = true;
        CellStack new_stack = stack;
        while (stuck && new_stack.size() > 0){
            Cell pop = new_stack.pop();
